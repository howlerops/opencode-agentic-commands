...
Return only the runner output. Bifrost requires the active OpenCode server for two-way no-gap sync, protects it with a Bifrost-managed local auth proxy, and must not silently start managed Web mode when no active server is available. Managed Web mode is explicit only: use start web when the user wants a separate browser portal without TUI/Web live sync. The username is opencode unless OPENCODE_SERVER_USERNAME was set by the runner environment. For start, status, and sync, keep the URL, username, password, Copy login, server mode, session links, and live TUI control API lines easy to see.`,
  },
  polaris: {
