...
- If a review subagent fails because its configured model is unavailable, retry with an available/current active model when possible; otherwise continue manually instead of stopping.`,
  ...
  bifrost: {
    ...
    render: (args) => `Open a Bifrost remote portal for this OpenCode workspace, or manage an existing portal.
...
State and logs:
- Use state directory: .opencode/bifrost.
...
7. Start OpenCode Web bound to 127.0.0.1. Prefer a background process with logs under .opencode/bifrost, for example: OPENCODE_SERVER_PASSWORD=<password> opencode web --hostname 127.0.0.1 --port <port>.
8. Wait until the local health check or root URL responds. If startup fails, show the last useful log lines and stop.
9. Start the public tunnel. Prefer Cloudflare Quick Tunnel with: cloudflared tunnel --url http://127.0.0.1:<port>.
