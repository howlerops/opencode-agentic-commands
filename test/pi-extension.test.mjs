...
assert.match(COMMANDS.munin.render("skills"), /not a single-pass run/)
assert.match(COMMANDS.munin.render("skills"), /metric\/outcome has plateaued/)
assert.match(COMMANDS.eitri.render("workflow"), /not a single-pass run/)
assert.match(COMMANDS.eitri.render("workflow"), /revise and re-test repeatedly/)
assert.match(COMMANDS.eitri.render("workflow"), /latest evaluation cannot identify another useful improvement/)
