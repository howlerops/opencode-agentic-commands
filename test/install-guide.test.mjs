...
const rawUrl = "https://raw.githubusercontent.com/howlerops/valhalla/refs/heads/main/docs/guide/installation.md"

assert.match(guide, /^# Installation/)
assert.match(guide, /For LLM Agents/)
assert.match(guide, /npm install --prefix "\$HOME\/.config\/opencode" github:howlerops\/valhalla/)
assert.match(guide, /npx @howlerops\/valhalla@latest/)
assert.match(guide, /pi install git:github.com\/howlerops\/valhalla/)
...
assert.match(docs, /npm install --prefix "\$HOME\/.config\/opencode" github:howlerops\/valhalla/)
