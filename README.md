...
HowlerOps Valhalla is an agentic slash-command suite for OpenCode and Pi. The package adds a compact command suite for planning, execution, research, agent creation, persistent work loops, review, remote access, and full end-to-end orchestration.
...
- `/bifrost` or `/bifrost start` uses the active OpenCode server behind the current TUI when it is available, places a Bifrost-managed authenticated local proxy in front of it, exposes that proxy through Cloudflare Quick Tunnel by default, and opens the current-session deep link when available.
- If the active server is unavailable in default `auto` mode, `/bifrost start` falls back to a separate managed Web server, reports the fallback reason, and prints an authenticated `opencode attach --session <current-session>` command for terminal attachment. Set `serverMode` to `active` when you want stale or missing active-server context to fail instead of falling back.
...
`heimdall` is a global wrapper for the long-running backend workflow. It forwards explicit opencode subcommands such as `run`, `models`, `session`, `web`, and `serve` unchanged. For TUI-style invocations, it starts or reuses a Bifrost-managed Web backend, then runs `opencode attach <local-url> --username ... --password ...` with any session/dir flags you provided. `opencode-heimdall` is also installed as a compatibility alias.
...
```json
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": [
    ["@howlerops/valhalla/hugin", { "maxParallelSubagents": 4 }],
    ["@howlerops/valhalla/tyr", { "baroModel": "openai/gpt-5.3-codex-spark" }],
    ["@howlerops/valhalla/munin", { "experimentCommand": "uv run train.py" }],
    ["@howlerops/valhalla/eitri", { "completionModel": "openai/gpt-5.3-codex-spark" }],
    ["@howlerops/valhalla/vidar", { "maxGoalLoops": 20 }],
    ["@howlerops/valhalla/skuld", { "maxReviewLoops": 10 }],
    ["@howlerops/valhalla/polaris", { "maxOrchestrationLoops": 20 }],
    ["@howlerops/valhalla/bifrost", { "stateDir": ".opencode/bifrost" }]
  ]
}
```
